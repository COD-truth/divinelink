...
export interface Payment {
  ...
  label: string;           // what this payment is for: "Consultation", "Extraction", etc
  amountDue: number;       // total price of this service
  amountPaid: number;      // amount paid so far
  balance: number;         // amountDue - amountPaid (auto-calculated)
  status: PaymentStatus;   // paid | partial | unpaid
  method: PaymentMethod;
  paidAt?: string;         // date/time of payment
  dueDate?: string;        // optional deadline for payment
  installments?: PaymentInstallment[]; // partial payments history
  notes?: string;
  clinicId?: string;
  createdAt: string;
  updatedAt: string;
...
export interface Drug {
  ...
  initialStock: number;    // stock when first added (to calculate what went out)
  unit: string;
  buyPrice: number;
  sellPrice: number;
  expiration?: string;
  ...
  status: DrugStatus;
  clinicId?: string;
  createdAt: string;       // exact date+time added
  updatedAt: string;
}
...
export interface DrugTransaction {
  id?: number;
  drugId: number;
  drugName: string;        // denormalized for history display
  drugCategory: string;
  drugUnit: string;
  type: TransactionType;   // "in" | "out"
  quantity: number;
  stockBefore: number;     // stock before this transaction
  stockAfter: number;      // stock after this transaction
  price: number;
  totalAmount: number;     // quantity * price
  patientId?: number;
  patientCode?: string;    // anonymous code
  paymentStatus?: PaymentStatus;
  paymentMode?: string;
  supplier?: string;       // for "in" transactions
  batchNumber?: string;
  reason?: string;         // reason for out: dispensed/expired/damaged/other
  performedBy?: string;    // doctor/user who did this
  notes?: string;
  clinicId?: string;
  createdAt: string;       // exact timestamp
}
